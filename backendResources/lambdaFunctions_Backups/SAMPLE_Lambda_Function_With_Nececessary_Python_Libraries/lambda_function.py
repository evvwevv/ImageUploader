            ...
            for i in range(len(resultArr)):
                ...
                if resultArr[i]['userName'] == passedInUserName:
                    ...
                    for n in newTags:
                        if n not in curImgDict:
                            ...
                            if curImgDict[n] == None:
                                ...
                                if newImage not in curImgDict[n]:
                                    curImgDict[n].append(newImage)
                    ...
                    for k in copyOfKeys:
                        ...
                        if k not in newTags: 
                            ...
                            if curImgDict[k] != None and len(curImgDict[k]) > 0:
                                ...
                                if newImage in curImgDict[k]:
                                    curImgDict[k].remove(newImage)
                                    logger.info("Removing image %s from key[tag] not found in new tags %s...", newImage, k)
                    ...
                    if newImage not in curImgDictByImage:
                        ...
                        if curImgDictByImage[newImage] == None:
                            newTagsList = list(passedInTags)
                            curImgDictByImage[newImage] = newTagsList
                            ...
                            for t in newTags:
                                if t not in curImgDictByImage[newImage]:
                                    curImgDictByImage[newImage].append(t)
                            ...
                            for t in curTagsListCopy:
                                if t not in newTags:
                                    curImgDictByImage[newImage].remove(t)
                                    logger.info("Removing tag %s from key (imagename) %s...", t, newImage)
                    ...
                    #ensure true owner of image
                    if newImage not in canViewDict:
                        usersThatCanView = []
                        usersThatCanView.append(curUserName)
                        canViewDict[newImage] = usersThatCanView
                    #else:
                    #    canViewDict[newImage].append(curUserName)
